...
class ModelErrorBoundary extends Component<{ children: ReactNode }, { error: Error | null }> {
  ...
  render() {
    if (this.state.error) return <ModelLoadError message={this.state.error.message} />;
...
function ModelLoadError({ message }: { message: string }) {
  return (
    <Html center>
      <div className="w-[290px] rounded-2xl border border-red-300/15 bg-[#07090a]/95 p-5 text-white shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-2 font-mono text-[8px] uppercase tracking-[0.2em] text-red-200/70">
          <span className="h-1.5 w-1.5 rounded-full bg-red-300" /> MODEL LOAD ERROR
        </div>
        <p className="mt-3 text-[11px] leading-5 text-white/55">NimbleBrain.glb could not be decoded by the browser.</p>
        <p className="mt-3 break-all font-mono text-[7px] leading-4 text-white/25">{message || MODEL_URL}</p>
      </div>
    </Html>
  );
}
...
function LoadingModel() {
  ...
  return (
    <Html center>
      <div className="w-[250px] rounded-2xl border border-cyan-200/10 bg-[#050809]/90 p-5 text-white shadow-2xl backdrop-blur-xl">
        <div className="flex items-center justify-between font-mono text-[8px] uppercase tracking-[0.2em] text-cyan-100/60">
          <span>Loading specimen</span><span>{Math.round(progress)}%</span>
        </div>
        <div className="mt-4 h-px overflow-hidden bg-white/10"><div className="h-full bg-cyan-300 transition-all" style={{ width: `${progress}%` }} /></div>
        <p className="mt-3 font-mono text-[7px] uppercase tracking-[0.16em] text-white/20">NIMBLE / NIH 3D ANATOMICAL MODEL</p>
      </div>
    </Html>
  );
}
...
function Model({ mode }: { mode: Mode }) {
  ...
  const normalized = useMemo(() => {
    ...
    clone.traverse((node) => {
      ...
      materials.forEach((raw) => {
        ...
        if ("roughness" in mat) mat.roughness = Math.min(0.72, Math.max(0.35, mat.roughness || 0.58));
        if ("metalness" in mat) mat.metalness = Math.min(0.08, mat.metalness || 0.02);
  ...
  return (
    <group>
      ...
      {mode === "CIRCUIT" && <CircuitLayer object={normalized.object} />}
...
function CircuitLayer({ object }: { object: THREE.Object3D }) {
  const edges = useMemo(() => {
    ...
    object.traverse((node) => {
      const mesh = node as THREE.Mesh;
      if (mesh.isMesh && mesh.geometry) result.push(new THREE.EdgesGeometry(mesh.geometry, 34));
    });
    return result;
  }, [object]);
  ...
  const network = useMemo(() => {
    ...
    for (let i = 0; i < 26; i++) {
      ...
      const r = 0.45 + (i % 5) * 0.055;
      points.push(new THREE.Vector3(Math.cos(a) * r * 1.2, Math.sin(a * 1.7) * 0.42, Math.sin(a) * r));
    }
    return points;
  }, []);
    ...
    <group>
      {edges.map((geometry, i) => (
        <lineSegments key={i} geometry={geometry}>
          <lineBasicMaterial color="#62e9ff" transparent opacity={0.12} depthWrite={false} />
        </lineSegments>
      ))}
      {network.slice(0, -1).map((p, i) => (
        <Line key={i} points={[p, network[i + 1]!]} color="#64e9ff" transparent opacity={0.55} lineWidth={0.8} depthWrite={false} />
      ))}
...
function NetworkNodes({ points }: { points: THREE.Vector3[] }) {
  ...
  return <group ref={ref}>{points.map((point, i) => <mesh key={i} position={point}><sphereGeometry args={[0.018, 10, 10]} /><meshBasicMaterial color={i % 3 === 0 ? "#b89cff" : "#62e9ff"} /></mesh>)}</group>;
}

function SignalLayer() {
  const nodes = useMemo(() => Array.from({ length: 42 }, (_, i) => new THREE.Vector3((Math.sin(i * 2.13) * 0.72), Math.cos(i * 1.37) * 0.5, Math.sin(i * 0.91) * 0.7)), []);
  return <group>{nodes.slice(0, -1).map((point, i) => <SignalLine key={i} start={point} end={nodes[i + 1]!} phase={i * 0.28} />)}{nodes.map((point, i) => <SignalNode key={i} position={point} phase={i * 0.37} />)}</group>;
}

function SignalLine({ start, end, phase }: { start: THREE.Vector3; end: THREE.Vector3; phase: number }) {
  const ref = useRef<THREE.Line>(null);
  useFrame((state) => { const material = ref.current?.material as THREE.LineBasicMaterial | undefined; if (material) material.opacity = 0.06 + (Math.sin(state.clock.elapsedTime * 2.2 + phase) + 1) * 0.09; });
  const geometry = useMemo(() => new THREE.BufferGeometry().setFromPoints([start, end]), [start, end]);
  return <line ref={ref} geometry={geometry}><lineBasicMaterial color="#927cff" transparent opacity={0.12} depthWrite={false} /></line>;
}

function SignalNode({ position, phase }: { position: THREE.Vector3; phase: number }) {
  const ref = useRef<THREE.Mesh>(null);
  useFrame((state) => { if (ref.current) ref.current.scale.setScalar(0.8 + (Math.sin(state.clock.elapsedTime * 3 + phase) + 1) * 0.35); });
  return <mesh ref={ref} position={position}><sphereGeometry args={[0.016, 10, 10]} /><meshBasicMaterial color="#aa91ff" /></mesh>;
}

function NavigationLayer() {
  return <group>{Array.from({ length: 26 }, (_, i) => <Agent key={i} index={i} />)}</group>;
}

function Agent({ index }: { index: number }) {
  const ref = useRef<THREE.Mesh>(null);
  const origin = useMemo(() => new THREE.Vector3(Math.sin(index * 1.77) * 0.85, Math.cos(index * 1.23) * 0.5, Math.sin(index * 0.71) * 0.75), [index]);
  useFrame((state) => {
    if (!ref.current) return;
    const t = state.clock.elapsedTime * 0.24 + index * 0.41;
    ref.current.position.set(origin.x + Math.sin(t) * 0.13, origin.y + Math.sin(t * 1.3) * 0.08, origin.z + Math.cos(t * 0.8) * 0.12);
  });
  return <mesh ref={ref} position={origin}><sphereGeometry args={[0.014, 8, 8]} /><meshBasicMaterial color="#62e9ff" /></mesh>;
}
...
function Scene({ mode }: { mode: Mode }) {
  return (
    <>
      ...
      <fog attach="fog" args={["#020506", 4.5, 8]} />
      <PerspectiveCamera makeDefault position={[0, 0.03, 3.8]} fov={34} />
      <ambientLight intensity={1.05} />
      <directionalLight position={[3, 4, 5]} intensity={2.4} color="#f4fcff" />
      <pointLight position={[-2.8, 1.4, 2]} intensity={8} distance={6} color="#1cbcff" />
      <pointLight position={[2.2, -1.6, 1]} intensity={6} distance={5} color="#7755ff" />
      <Environment preset="night" />
      <Model mode={mode} />
      <OrbitControls enablePan={false} enableDamping dampingFactor={0.055} minDistance={2.0} maxDistance={7} autoRotate autoRotateSpeed={0.13} />
    </>
  );
}
      ...
      <div className="relative mx-auto max-w-[1540px] px-[clamp(1.25rem,4vw,4rem)] py-[clamp(5rem,9vw,8rem)]">
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_330px]">
          <div className="relative h-[min(72vh,760px)] min-h-[560px] overflow-hidden rounded-[28px] border border-white/[0.09] bg-[#030708] shadow-[0_30px_120px_rgba(0,0,0,.55)]">
            <ModelErrorBoundary>
              <Canvas dpr={[1, 1.5]} gl={{ antialias: true, alpha: false }} camera={{ position: [0, 0.03, 3.8], fov: 34 }}>
                <Suspense fallback={<LoadingModel />}><Scene mode={active} /></Suspense>
              </Canvas>
            </ModelErrorBoundary>
            ...
            <div className="pointer-events-none absolute right-6 top-6 text-right"><p className="font-mono text-[7px] uppercase tracking-[0.25em] text-white/20">MODEL</p><p className="mt-1 font-mono text-[9px] text-white/45">NIMBLEBRAIN.GLB</p></div>
